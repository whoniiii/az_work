...
`~/.claude/skills/azure-arch-builder/scripts/generate_html_diagram.py`를 실행하여 인터랙티브 HTML 다이어그램을 만든다.

```bash
python ~/.claude/skills/azure-arch-builder/scripts/generate_html_diagram.py \
  --services '<JSON>' \
  --connections '<JSON>' \
  --title "아키텍처 제목" \
...
다음 정보가 확정되면 Phase 2로 넘어간다:

**필수 확정 항목:**
- [ ] 서비스 목록 및 SKU
- [ ] 네트워킹 방식 (Private Endpoint 여부)
- [ ] 리소스 그룹 이름 및 위치 (기본: koreacentral)
- [ ] 구독 ID (배포 시 필요)
