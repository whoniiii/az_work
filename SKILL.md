...
> **중요**: 초안 제안 후 추가 질문을 여러 개 나열하지 않는다. 사용자가 원하는 변경이 있으면 스스로 말한다.
> 네트워킹 방식, 인덱싱 방법, 사용 규모 등 세부 질문은 사용자가 먼저 피드백을 주면 그때 필요한 것만 물어본다.
