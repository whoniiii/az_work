...
Agent 도구로 서브에이전트를 백그라운드(`run_in_background: true`)로 실행하면 완료 시 **자동으로 알림**이 온다.
절대로 아래와 같은 행위를 하지 않는다:
...
**올바른 사용법:**
```
// 백그라운드 실행 → 알림 올 때까지 다른 작업 진행 또는 대기
Agent({ prompt: "...", run_in_background: true })
// 완료 알림이 오면 결과를 자연스럽게 사용
```

이 규칙을 어기면 사용자에게 불필요한 권한 요청이 발생하고 내부 구조에 의존하는 취약한 코드가 된다.
